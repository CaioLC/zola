...
fn image_op_test(
    ...
    let resp = proc.enqueue(imageproc::ImageOperation::Resize(resize_op), source_img.into(), source_path, format, None).unwrap();
...
fn resize_and_check(source_img: &str) -> bool {
    ...
    let resp = proc.enqueue(imageproc::ImageOperation::Resize(resize_op), source_img.into(), source_path, "jpg", None).unwrap();
